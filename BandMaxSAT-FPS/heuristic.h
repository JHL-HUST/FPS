...
void BandMSFPS::init(vector<int> &init_solution)
{
    local_times = 0;
    if_exceed = 0;
...
int BandMSFPS::pick_var()
{
    ...
    if (clause_lit_count[sel_c] <= sc_num){
        for (int i = 0; i < clause_lit_count[sel_c]; i++){
            best_vars[i] = clause_lit[sel_c][i].var_num;
        }
        selected_nums = clause_lit_count[sel_c];
    }
    else{
        selected_nums = sc_num;
        int best_vars_num = 0;
        for (int i = 0; i < sc_num; i++){
            best_vars[best_vars_num] = clause_lit[sel_c][rand() % clause_lit_count[sel_c]].var_num;
...
void BandMSFPS::local_search(char *inputfile)
{
...
void BandMSFPS::local_search_with_decimation(char *inputfile)
{
    settings();
...
void BandMSFPS::increase_weights()
{
    int i, c, v;
...
void BandMSFPS::update_clause_weights()
{
...
void BandMSFPS::update_clause_scores(long long s)
{
    int i, j;
    double stemp;
